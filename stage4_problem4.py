...
基于问题 3 输出的 10 Hz 融合轨迹，对射击和拍照两类任务
进行可行窗口搜索与贪心调度，最终输出任务执行时刻表。

处理流程：
  1. 读取 10 Hz 融合轨迹 (Problem3_10Hz.xlsx)
  2. 读取目标点坐标（每个目标可同时尝试射击和拍照）
  3. 计算速度、加速度等运动状态
  4. 实例化约束检查器，搜索所有可行时间窗口
  5. 贪心调度，生成无冲突的任务序列
  6. 保存结果至 Excel，打印汇总信息

依赖：config.py, core/motion_utils.py,
      core/constraint_checker.py, core/task_scheduler.py
"""
...
def load_trajectory() -> tuple:
    ...
    print(f"[Problem4] 加载轨迹: {traj_path}")
...
def load_targets() -> list:
    ...
    每个目标可同时尝试射击和拍照，不再按前缀预分类。

    Returns
    -------
    all_targets : list of dict
        每个元素 {'id': int, 'name': str, 'x': float, 'y': float}。
    """
    ...
    print(f"[Problem4] 目标总数: {len(all_targets)} 个")
...
def plot_problem4_results(
    ...
    ax.set_title(
        f"Problem 4 — Task Schedule ({len(scheduled_tasks)} tasks)",
    ...
    if len(scheduled_tasks) > 0:
        ...
        for i, task in enumerate(scheduled_tasks):
            ...
            task_label = "Shoot" if task["task_type"] == "shoot" else "Photo"
    ...
    print(f"[Problem4] 图表已保存至 {figures_dir}")
...
if __name__ == "__main__":
    ...
    print(f"  加速度: min={np.min(acc):.3f}, max={np.max(acc):.3f}, "
          f"mean={np.mean(acc):.3f} m/s^2")

    # ======================================================
    # Step 4: 约束检查器实例化
    # ======================================================
    print("\n" + "=" * 60)
    print("  Step 4: 约束检查器实例化")
    print("=" * 60)

    checker = ConstraintChecker(t, x, y, vx, vy, ax, ay)
    print(f"  dt = {checker.dt:.4f} s, {checker.n} 采样点")
    ...
    # ======================================================
    # Step 5: 搜索所有可行窗口（所有目标 × 两种任务）
    # ======================================================
    print("\n" + "=" * 60)
    print("  Step 5: 搜索可行时间窗口")
    print("=" * 60)
    ...
    if len(windows_all) == 0:
        ...
        print(f"[Problem4] 空结果已保存至 {output_dir / 'result.xlsx'}")
        exit(0)
    ...
    if template_path.exists():
        try:
            ...
            wb.save(xlsx_path)
            print(f"[Problem4] 结果已保存至 {xlsx_path} (模板格式)")
        except Exception as e:
            print(f"[Problem4] 模板写入失败 ({e})，使用默认格式")
            df_result.to_excel(xlsx_path, index=False, engine="openpyxl")
            print(f"[Problem4] 结果已保存至 {xlsx_path}")
    else:
        df_result.to_excel(xlsx_path, index=False, engine="openpyxl")
        print(f"[Problem4] 结果已保存至 {xlsx_path}")

    size_kb = xlsx_path.stat().st_size / 1024
    print(f"[Problem4] 文件大小: {size_kb:.1f} KB")
